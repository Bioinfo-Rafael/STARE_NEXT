...
KYOTO_DST_URL = "https://wdc.kugi.kyoto-u.ac.jp/dst_final/{yyyymm}/index.html"


def probe_dst(output_dir: str) -> dict:
    url = KYOTO_DST_URL.format(yyyymm="202001")
...
def fetch_dst(start_date: str, end_date: str, output_dir: str) -> list[dict]:
    ...
    for yyyymm in month_iter(start_date, end_date):
        url = KYOTO_DST_URL.format(yyyymm=yyyymm)
        response, sample = get(url, timeout=30)
        if response is None or response.status_code != 200:
            append_jsonl(f"{output_dir}/kp_dst_failures.jsonl", {"source": "dst", "month": yyyymm, "status": "fetch_error", "sample_http": sample.as_dict()})
            continue
        rows = _parse_month_html(yyyymm, response.text)
        if not rows:
            append_jsonl(f"{output_dir}/kp_dst_failures.jsonl", {"source": "dst", "month": yyyymm, "status": "parse_empty", "sample_http": sample.as_dict()})
            continue
