...
def combined_negative_control(xs: list[float], ys: list[float], dates: list[str]) -> tuple[float | None, bool]:
    vals = [v for v in [time_shuffle_correlation(xs, ys), block_shuffle_by_month_correlation(xs, ys, dates), year_within_doy_shuffle_correlation(xs, ys, dates)] if v is not None]
