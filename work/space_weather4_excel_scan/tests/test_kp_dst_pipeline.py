...
from src.modeling.evaluation import candidate_filter, reject_reasons
...
class KpDstPipelineTests(unittest.TestCase):
    ...
    def test_aggregate_target_type_overrides_registry_guess(self) -> None:
        ...
        for day in range(1, 11):
            ...
            feature_rows.append(
                {
                    "date": date,
                    "external_source_group": "kp_dst",
                    "external_feature_name": "kp_mean_daily",
    ...
    def test_raw_feature_is_not_strict_candidate(self) -> None:
        row = {
            "lag_direction": "external_precedes_target",
            "feature_adjustment": "raw",
            "negative_control_pass": True,
            "fdr_q_value": 0.01,
            "correlation": 0.2,
            "negative_control_correlation": 0.01,
            "n_samples": 500,
            "best_reverse_lag_score": 0.01,
            "precursor_minus_reverse_score": 0.19,
        }
        self.assertFalse(candidate_filter(row))
        self.assertIn("fail_raw_feature", reject_reasons(row))

    def test_reverse_lag_stronger_is_not_strict_candidate(self) -> None:
        row = {
            "lag_direction": "external_precedes_target",
            "feature_adjustment": "seasonal_adjusted",
            "negative_control_pass": True,
            "fdr_q_value": 0.01,
            "correlation": 0.2,
            "negative_control_correlation": 0.01,
            "n_samples": 500,
            "best_reverse_lag_score": 0.25,
            "precursor_minus_reverse_score": -0.05,
        }
        self.assertFalse(candidate_filter(row))
        self.assertIn("fail_reverse_lag", reject_reasons(row))
