...
def brier_score(xs: list[float], ys: list[float]) -> float | None:
    ...
    return sum((1 / (1 + math.exp(-max(min(z, 35), -35))) - y) ** 2 for z, y in zip(zs, labels)) / len(labels)


def logistic_fit(xs: list[float], ys: list[float], steps: int = 15, lr: float = 0.05) -> tuple[float | None, float | None]:
    labels = [1.0 if y > 0 else 0.0 for y in ys]
    if len(set(labels)) < 2 or len(set(xs)) < 2:
        return None, None
    mx = mean(xs)
